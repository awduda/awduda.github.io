...
function continue_operations(){
  ...
  if (selected_image+1==image_info_list.length){
    ...
    $("#continue").click(function(){
      ...
      for (i=0; i<=image_info_list.length-1;i++){
      json=JSON.stringify(image_info_list[i])
      $.ajax({
          url: '/add_training/',
          type: 'POST',
...
img=nj.images.read($(".operating_image")[0])
...
for (i=0; i<image_info_list[selected_image].thickness_data.length; i++){

  contrast_list=[]
  point_location=[Math.round(parseFloat(image_info_list[selected_image].thickness_data[i][1])),Math.round(parseFloat(image_info_list[selected_image].thickness_data[i][2]))]

    for (i=0; i<=2;i++){
      layer=img.slice([0,img.shape[0]],[0,img.shape[1]],[i,i+1])
      layer=nj.images.rgb2gray(layer)
      bg_val=layer.get(bg_location[1],bg_location[0],0)
      fg_val=layer.get(point_location[1],point_location[0],0)
